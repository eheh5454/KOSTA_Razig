...
void reserv()
{
    ...
    time_t ct;
    struct tm tm;
    char local_hour[BUF_SIZE];
    char local_min[BUF_SIZE];
    char local_time[BUF_SIZE];
    ...
    sprintf(local_hour, "%d", hour);
    sprintf(local_min, "%d", min);
    ...
    strcat(local_hour,local_min);

    strcpy(local_time,local_hour);

    printf("local time: %s\n", local_time);
    printf("reserv time:%s\n", reserv_time);

    if(strcmp(local_time,reserv_time) == 0)
        auto_move =1;
...
void* UDP_Server(void *arg)
{
	...
	while(1)
	{
        //for check cmd
        char buf[BUF_SIZE];
        char check_time[BUF_SIZE];

        printf("Waiting.......\n");
        str_len = recvfrom(sock, buf, BUF_SIZE-1, 0, (struct sockaddr*)&clnt_adr, &clnt_adr_sz);
        buf[str_len] = '\0';
        printf("%s\n", buf);
        strncpy(check_time, buf, 4);

        if(strcmp(check_time, "time") == 0)
        {
            check_reserv = 1;
            strncpy(reserv_time,buf+4,4);
            continue;
        }
...
void loop(void){
 ...
 if(serialDataAvail (fd)){
   ...
   // {
    ...
    if(ultval < 20 && valcheck>_valcheck && valcheck != 0 && avoidmodeout == 0)
    {
        if(endmode < 2) {
        _valcheck = valcheck;
        avoidmode = 1;
    ...
    if(avoidmode == 0)
    {
        if(clrval >= clrvalmax && valcheck2 != 0 )
        {
        motor_go();
        gocheck++;
        printf("%d\n",gocheck);
        ...
        else if(valcheck2 > 1 && valcheck2 != 0)
        {
            if(gocheck < 15)
            {
                while(clrval < clrvalmax && _valcheck2 <valcheck2 )
                {
